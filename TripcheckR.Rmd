---
title: "Predicting Santiam Pass Accidents using Twitter"
...
The [Oregon Dept of Transportation](https://tripcheck.com/Pages/Twitter.asp) regularly publishes, as a public service, live road reports via Twitter. The record created by these tweets can be used to recontruct both the times and locations of accidents on sections of road and also to correlate them with road conditions.  

This analysis is useful to drivers who intend to cross the pass since at least in a few incidents otherwise innocent drivers become entangled in accidents caused by other drivers who lose control of their cars. Avoiding times when these accidents are most frequent can increase safety.
...
We can also look at the correlation of accidents to the Day of Week. 
...
cdt<- factor(cdt, levels= c("Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"))
...
However, with a little number crunching we can show that the probability that there was an accident given there was snow equal to (the number of days when there was snow and an accident) divided by (the number of days when there was snow) is `r nCommRate`. 
That means that for roughly one in four days there is snow on the pass, there is an accident on this 11 mile stretch of road. 
