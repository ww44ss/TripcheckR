...
This benefits drivers since they have no other way to learn about road safety in any quantitative way. This is especially useful to careful or experienced drivers who might become entangled in accidents caused by others. Avoiding times when these accidents are most frequent can increase peronal safety.
...
Over the period of study there were `r 7 * nrow(plotdata)/as.numeric((rangeYM[2]- rangeYM[1]))` accidents per week, meaning there is about one accident every two weeks when averaged over the entire record. Since the distance over which the accidents occured is the relatively short distance of `r max(plotdata$distance)-min(plotdata$distance)` miles, the accident density is `r 52*7 * nrow(plotdata)/as.numeric((rangeYM[2]- rangeYM[1]))/(max(plotdata$distance)-min(plotdata$distance))` accident/year/mile. This compares to the total number of crashes `r TotalCrashes` reported along the 123 miles of highway covered by the `r file_name` data or `r 52*7 * (TotalCrashes)/as.numeric((rangeYM[2]- rangeYM[1]))/123`  accident/year/mile. Removing the Santiam Pass data the accident rate drops to `r 52*7 * (TotalCrashes-nrow(plotdata))/as.numeric((rangeYM[2]- rangeYM[1]))/(123-11)` over `r 123-11` miles.

During this period there were `r nSnow` days with snow reported on the road. The rest of this analysis will look at how this affects the accident rate. 
...
The above histrogram shows substantial variation depending on the day of the week. Friday, in this anlaysis has almost four times the number of accidents than Wednesday. The mean number of accidents per weekday is `r cdmean` and the standard deviation is `r cdsd`. The minimum number of accidents is `r min(cdt$Freq)` and the maximum is `r max(cdt$Freq)`, having a ratio of `r max(cdt$Freq)/min(cdt$Freq)`:1.
...
PAgivenS <- nCommRate*SnowRate/AccRate
...
We can also use Bayes's formula to show that the probability that there is snow on the road given as accident has occured is `r PAgivenS`. Given there was snow on the road only `r 100*SnowRate`% of days this is a substantial amplification of risk.   
